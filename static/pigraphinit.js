function pigraphinit() {
    if (!!window.EventSource) {
        ...
        var esource = new EventSource("/pistatus?fields=cpu_busy&fields=cpu_temp");
        esource.addEventListener("message", function(e) {
            var newinfo=JSON.parse(e.data);
            tempdataset.append(Date.now(),newinfo.cpu_temp);
            cpudataset.append(Date.now(), newinfo.cpu_busy*100);
