...
#    {'name': 'countin',     '_cclass': pvars.enumVar,       'fallbackValue': 'today', 'vlist': ['today','yesterday','this month', 'all']},
    {'name': 'diskfree',    '_cclass': pvars.floatVar,      'fallbackValue': 0},
)

class fileManager(papp.appManager):
    def __init__(self, basefolder, childdefs=None, **kwargs):
        self.basefoldervar=basefolder
        super().__init__(name='fileman', childdefs=vardefs, **kwargs)
        self.currentfiles=None
        self.filetime=time.time()
        self.cq=queue.Queue()
        self.running=True
        self.fthread=threading.Thread(name=self.name, target=self.slowthread)
        self.fthread.start()
    ...
    def slowthread(self):
        ...
        while self.running:
            if cmd is None:
                ...
                self['diskfree'].setValue(nv,'driver')
            try:
